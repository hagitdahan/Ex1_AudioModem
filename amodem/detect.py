...
class Detector:
    ...
    COHERENCE_THRESHOLD = 0.75  # Relaxed from 0.9 to 0.75 for better weak signal detection
    ...
    def __init__(self, config, pylab):
        ...
        self.max_offset = config.timeout * 2 * config.Fs  # Increased timeout
        self.plt = pylab
    ...
    def _wait(self, samples):
        ...
        for offset, buf in common.iterate(samples, self.Nsym, index=True):
            ...
            print(f"Offset {offset}: Coherence = {abs(coeff):.2f}")  # Debug print

            if abs(coeff) > self.COHERENCE_THRESHOLD:
                counter = min(counter + 1, self.CARRIER_THRESHOLD)
            else:
                counter = max(counter - 1, 0)  # Instead of full reset
    ...
    def run(self, samples):
        ...
        print(f"Carrier detected at ~{start_time * 1e3:.1f} ms")  # Debug
        ...
        print(f"Start offset: {offset}, adjusted time: {start_time * 1e3:.2f} ms")  # Debug
    ...
    def find_start(self, buf):
        ...
        print("Correlation peak index:", index)  # Debug
